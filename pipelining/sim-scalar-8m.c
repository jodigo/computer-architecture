...
void fetch(void)
{
    ...
    if( g_piperegister[IF_IS_REGISTER] != NULL )
    ...
    if( g_fetch_redirected ) {
       ...
       pI->status = FETCHED; 
       g_piperegister[IF_IS_REGISTER] = pI; 
    }
}
...
void decode(void)
{
    ...
    inst_t *pI = g_piperegister[IF_IS_REGISTER];

    if( g_piperegister[IS_RF_REGISTER] != NULL )
        return; // stall
    if( pI == NULL )
        return; // bubble
...
#include "machine.def"
    ...
    pI->stalled = 0;
    pI->status = DECODED;
    g_piperegister[IS_RF_REGISTER] = pI; // move to ID/EX register
    g_piperegister[IF_IS_REGISTER] = NULL;
}

void rf(void)
{
    inst_t *pI = g_piperegister[IS_RF_REGISTER];
    if( g_piperegister[RF_EX_REGISTER] != NULL )
        return; // stall
    if( pI == NULL )
        return; // bubble, nothing to do

    pI->status = RFED;
    g_piperegister[RF_EX_REGISTER] = pI; 
    g_piperegister[IS_RF_REGISTER] = NULL;
}

void execute()
{
    inst_t *pI = g_piperegister[RF_EX_REGISTER];
    if( g_piperegister[EX_DF_REGISTER] != NULL )
        return; // stall
    if( pI == NULL )
        return; // bubble
    pI->stalled = 0;
    ...
    pI->status = EXECUTED;
    g_piperegister[EX_DF_REGISTER] = pI; // move to EX/MEM register
    g_piperegister[RF_EX_REGISTER] = NULL;
}

void memory()
{
    inst_t *pI = g_piperegister[EX_DF_REGISTER];
    if( g_piperegister[DF_DS_REGISTER] != NULL )
        return; // stall
    if( pI == NULL )
        return; // bubble, nothing to do

    pI->status = MEMORY_STAGE_COMPLETED;
    g_piperegister[DF_DS_REGISTER] = pI; // move to MEM/WB register
    g_piperegister[EX_DF_REGISTER] = NULL;
}

void ds() {
    inst_t *pI = g_piperegister[DF_DS_REGISTER];
    if( g_piperegister[DS_TC_REGISTER] != NULL )
        return; // stall
    if( pI == NULL )
        return; // bubble, nothing to do

    pI->status = DSED;
    g_piperegister[DS_TC_REGISTER] = pI; // move to MEM/WB register
    g_piperegister[DF_DS_REGISTER] = NULL;
}

void tc() {
    inst_t *pI = g_piperegister[DS_TC_REGISTER];
    if( g_piperegister[TC_WB_REGISTER] != NULL )
        return; // stall
    if( pI == NULL )
        return; // bubble, nothing to do

    pI->status = TCED;
    g_piperegister[TC_WB_REGISTER] = pI; // move to MEM/WB register
    g_piperegister[DS_TC_REGISTER] = NULL;
}

void writeback(void)
{
    inst_t *pI = g_piperegister[TC_WB_REGISTER];
    if( pI == NULL )
        return; // bubble, nothing to do
    ...
    g_piperegister[TC_WB_REGISTER] = NULL;
    free_inst(pI);
...
void display_pipeline()
{
    ...
    printf("PC     :=             0x%6x\n", g_fetch_pc );
    printf("IF/IS  := ");
    print_instruction( g_piperegister[IF_IS_REGISTER] );
    printf("IS/RF  := ");
    print_instruction( g_piperegister[IS_RF_REGISTER] );
    printf("RF/EX  := ");
    print_instruction( g_piperegister[RF_EX_REGISTER] );
    printf("EX/DF  := ");
    print_instruction( g_piperegister[EX_DF_REGISTER] );
    printf("DF/DS := ");
    print_instruction( g_piperegister[DF_DS_REGISTER] );
    printf("DS/TC := ");
    print_instruction( g_piperegister[DS_TC_REGISTER] );
    printf("TC/WB := ");
    print_instruction( g_piperegister[TC_WB_REGISTER] );
    printf("\n");
}
